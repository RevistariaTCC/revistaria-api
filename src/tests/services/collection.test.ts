...
describe('Collections Services', () => {
  ...
  describe('CreateCollection()', () => {
    it('Should return the generated collection', async () => {
      const spyCreate =
        prismaAdapter.collection.create.mockResolvedValue(collection);
  ...
  describe('DeleteCollection()', () => {
    it('Should delete a existing collection', async () => {
      ...
      const spyDelete =
        prismaAdapter.collection.delete.mockResolvedValue(collection);
    ...
    it('Should return collection not found when id doesnt exists', async () => {
      ...
      await expect(deleteCategory.execute(collection.id)).rejects.toThrowError(
  ...
  describe('ListCollections()', () => {
    it('Should return list of collections', async () => {
      const spyFind = prismaAdapter.collection.findMany.mockResolvedValue([
        collection
      ]);
  ...
  describe('UpdateCollection()', () => {
    it('Should return the updated collection', async () => {
      ...
      const spyFind =
        prismaAdapter.collection.findUnique.mockResolvedValue(collection);
      const spyUpdate =
        prismaAdapter.collection.update.mockResolvedValue(updatedCollection);
    ...
    it('Should return collection not found when id doesnt exists', async () => {
      ...
      await expect(
        updateCategory.execute({ id: collection.id, name: 'Kimetsu' })
      ).rejects.toThrowError('Collection not found!');
      expect(spyFind).toHaveBeenCalledTimes(1);
    });
  });
});
