...
describe('User Services', () => {
  ...
  describe('GetUserById()', () => {
    ...
    it('Should return user not found when id doesnt exists', async () => {
      ...
      await expect(getUserById.execute(user.id)).rejects.toThrowError(
        'User not found.'
      );
      expect(spyFind).toHaveBeenCalledTimes(1);
    });
  ...
  describe('LinkCollections()', () => {
    ...
    it('Should connect user collections', async () => {
      const spyFind = prismaAdapter.user.findUnique.mockResolvedValue(user);
      const spyUpdate =
        prismaAdapter.user.update.mockResolvedValue(updatedUser);
      const linkCollection = new LinkCollection();

      const result = await linkCollection.connect({
    ...
    it('Should return user not found when id doesnt exists on connect', async () => {
      ...
      await expect(
        linkCollection.connect({ collectionID: 'uuid-test-1', user })
    ...
    it('Should disconnect user collections', async () => {
      const spyFind = prismaAdapter.user.findUnique.mockResolvedValue(user);
      const spyUpdate = prismaAdapter.user.update.mockResolvedValue({
        ...user,
        collections: []
      } as User);
      const linkCollection = new LinkCollection();

      const result = await linkCollection.disconnect({
        collectionID: 'uuid-test-1',
        user
      });
    ...
    it('Should return user not found when id doesnt exists on disconnect', async () => {
      ...
      await expect(
        linkCollection.disconnect({ collectionID: 'uuid-test-1', user })
      ).rejects.toThrowError('User not found.');
      expect(spyFind).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('LinkInterests()', () => {
    ...
    it('Should connect user interests', async () => {
      ...
      const result = await linkInterests.connect({
        categoryIDs: ['uuid-test-1'],
        user
      });
    ...
    it('Should return user not found when id doesnt exists on connect', async () => {
      ...
      await expect(
        linkInterests.connect({ categoryIDs: ['uuid-test-1'], user })
      ).rejects.toThrowError('User not found.');
      expect(spyFind).toHaveBeenCalledTimes(1);
    });
    ...
    it('Should disconnect user interests', async () => {
      ...
      const result = await linkInterests.disconnect({
        categoryIDs: ['uuid-test-1'],
        user
      });
    ...
    it('Should return user not found when id doesnt exists on disconnect', async () => {
      ...
      await expect(
        linkInterests.disconnect({ categoryIDs: ['uuid-test-1'], user })
      ).rejects.toThrowError('User not found.');
      expect(spyFind).toHaveBeenCalledTimes(1);
    });
  });
