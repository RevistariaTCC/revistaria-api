...
describe('Category Services', () => {
  ...
  describe('DeleteCategory()', () => {
    ...
    it('Should return category not found when id doesnt exists', async () => {
      ...
      await expect(deleteCategory.execute(category.id)).rejects.toThrowError(
        'Category not found!'
      );
      expect(spyFind).toHaveBeenCalledTimes(1);
    });
  ...
  describe('ListCategories()', () => {
    it('Should return list of categories', async () => {
      const spyFind = prismaAdapter.category.findMany.mockResolvedValue([
        category
      ]);
  ...
  describe('UpdateCategory()', () => {
    ...
    it('Should return category not found when id doesnt exists', async () => {
      ...
      await expect(
        updateCategory.execute({ id: category.id, name: 'Aventura' })
